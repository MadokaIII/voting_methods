 ...
 *
 * This function compares the token against a dynamically generated set of valid data column
 * identifiers. The range of valid identifiers is from "-1" to the value of `num_candidates`.
 * It is used to determine whether a token in a CSV file represents a data column.
 * The inclusion of `num_candidates` allows flexibility in the range of valid identifiers,
 * adapting to different contexts where the number of candidates (or categories) may vary.
 *
 * @param[in] token The string token to check.
 * @param[in] num_candidates The maximum number of valid data column identifiers,
 *             indicating the upper bound of the range starting from 0.
 ...
 * which indicates the start of data columns. The valid data tokens are dynamically determined
 * based on the number of candidates, ranging from "-1" to `nb_candidates`. The function assumes
 * that the first line contains headers and skips it. This functionality is particularly useful
 * in processing CSV files where the format or number of columns is not fixed and depends on the
 * context (e.g., number of candidates in an election).
 *
 * @param[in] file The file pointer to the CSV file.
 * @param[in] nb_candidates The number of candidates, defining the range of valid data columns.
 ...
 *
 * This function determines whether a given string matches the specific format used
 * for special column names in a CSV file. The expected format is "Q00_Vote-><number> - <name>".
 * It checks for the presence of "Q00_Vote->" followed by a sequence of digits and then " - ".
 *
 ...
 *
 * This function reads the first line of a CSV file to extract column names. It supports
 * a special format where the column names are prefixed with a specific pattern ("Q00_Vote-><number>
 * - "), and in such cases, only the part after " - " is extracted as the column name. The function
 * dynamically counts the number of columns and allocates memory for storing these names. It also
 * considers a starting position, allowing for the exclusion of certain columns from the beginning.
 *
 * @param[in] file The file pointer to the CSV file.
 * @param[out] columns_name A pointer to an array of strings where the extracted column names will
 * be stored.
 * @param[out] cols A pointer to store the number of columns.
 * @param[in] start_pos The position in the line from where to start considering columns as valid.
 *
 * @pre
 ...
 *
 * @post
 *   - columns_name contains the extracted column names, adjusted for any special formatting.
 ...
 *
 * Opens a CSV file, determines the starting position of data columns based on the number of
 * candidates, extracts column names, counts the number of data rows, and populates a matrix
 * with the data. It leverages other helper functions, like `get_start_pos`, to accomplish these
 * tasks. The range of valid data columns is defined by the `nb_candidates` parameter, allowing
 ...
 *   - The file format is expected to be a standard CSV with data separated by commas.
 *
 * @post
...
/**
 * @brief Finds the minimum integer value in an array, excluding a specific value.
 *
 * This function iterates through an integer array to find the minimum value. It
 * ignores any elements that have a value of -1, which is considered a special case
 * and not included in the comparison. The function is particularly useful in scenarios
 * where certain array elements are marked as -1 to indicate a special condition, such
 * as exclusion from a calculation or being invalid data points.
 *
 * @param[in] array Pointer to the integer array.
 * @param[in] size The size of the array.
 * @param[in] nb_candidates The number of candidates, used to determine the initial
 *                          minimum value (nb_candidates + 1).
 *
 * @return The minimum integer value found in the array, excluding any -1 values.
 *         If all values are -1, or if the array is empty, returns nb_candidates + 1.
 *
 * @pre
 *   - array must be a valid pointer to an integer array.
 *   - size should be greater than or equal to 0.
 *   - nb_candidates should be an integer representing the number of candidates in a context.
 *
 * @post
 ...
 *
 * @note
 *   - The value of nb_candidates + 1 is used as the initial minimum value to ensure
 *     that any valid integer in the array will be considered in the comparison.
 *   - If the array contains only -1 or is empty, the function returns nb_candidates + 1
 *     to indicate no valid minimum value was found.
 */
