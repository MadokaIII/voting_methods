 ...
 *
 * Allocates memory for a new StringBuffer and initializes it with the provided initial string.
 * If initialString is NULL, the StringBuffer is initialized as an empty string. The size parameter
 * specifies the length of the initial string to be copied.
 *
 ...
 * @return A pointer to the newly allocated StringBuffer, or NULL if memory allocation fails.
 ...
 *
 * Initializes a stack-allocated StringBuffer with the provided initial string up to the specified
 * size. If initialString is NULL, the StringBuffer is initialized as an empty string.
 *
 ...
 * @pre stringBuffer must not be NULL and size must be less than MAX_STRING_SIZE.
 * @post stringBuffer is initialized with the content of initialString or as an empty string.
 */
 ...
 *
 * Appends a string of specified size to the end of the StringBuffer. Resizes the buffer if
 * necessary.
 *
 ...
 * @pre stringBuffer must not be NULL and its size with the new string must be less than
 *      MAX_STRING_SIZE.
 ...
 *
 * Appends the content of the source StringBuffer to the end of the target StringBuffer.
 *
 * @param[in,out] target The target StringBuffer to append to.
 * @param[in] source The source StringBuffer to append from.
 * @return 0 on success, -1 on failure.
 * @pre Both target and source must not be NULL and their combined size must be less than
 *      MAX_STRING_SIZE.
 ...
 *
 * Frees the internal string memory and resets the size of the StringBuffer to 0.
 *
 * @param[in,out] stringBuffer The StringBuffer to clear.
 * @pre stringBuffer must not be NULL.
 * @post stringBuffer's internal string memory is freed and its size is set to 0.
 */
 ...
 *
 * Prints the contents of the StringBuffer using the specified output format and separator.
 *
 ...
 * @pre stringBuffer must not be NULL.
 */
void print_stringbuffer(const ptrStringBuffer stringBuffer, uint outputType, const char *separator);
 ...
 *
 * Frees the memory used by the internal string of a stack-allocated StringBuffer.
 *
 * @param[in,out] stringBuffer The stack-allocated StringBuffer whose internal string is to be
 *                             freed.
 * @pre stringBuffer must not be NULL and must have a dynamically allocated internal string.
